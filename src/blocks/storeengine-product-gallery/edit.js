...
import { getGallaryImageContainerCSS, getGallaryImageCSS } from './styling';
import './editor.scss';
...
export default function Edit( props ) {
	...
	const generatedCSS = useMemo( () => {
		...
		cssGenerator.addClassStyles(
			...
			getGallaryImageCSS( attributes ),
			getGallaryImageCSS( attributes, 'Tablet' ),
			getGallaryImageCSS( attributes, 'Mobile' )
		);
		cssGenerator.addClassStyles(
			`{{WRAPPER}} .storeengine-gallery_wrapper .storeengine-row .storeengine-col-12 `,
			getGallaryImageContainerCSS( attributes ),
			getGallaryImageContainerCSS( attributes, 'Tablet' ),
			getGallaryImageContainerCSS( attributes, 'Mobile' )
		);
