...
document.addEventListener( 'DOMContentLoaded', function () {
	...
	const MapBlockRender = ( element, Settings ) => {
		...
		Settings.mapMarkerList.forEach( function ( item ) {
			...
			if ( item.iconType === 'custom' ) {
				...
				const icon = new LeafIcon( { iconUrl: item.customIconUrl } );
		...
		const mapType =
			Settings.mapType === 'GM_SATELLITE' ? GM_SATELLITE : GMType;
		...
		if (
			Settings.mapMarkerList.length &&
			Settings.mapMarkerList[ Settings.centerIndex ]
		) {
			config.center = [
				Settings.mapMarkerList[ Settings.centerIndex ].lat,
				Settings.mapMarkerList[ Settings.centerIndex ].lng,
			];
		}
	...
	if ( mapBlockWrapper.length ) {
		mapBlockWrapper.forEach( ( element ) => {
			MapBlockRender(
				element,
				JSON.parse( element.getAttribute( 'data-settings' ) )
			);
		} );
	}
} );
