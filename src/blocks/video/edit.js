...
import { getVideoContainerCSS } from './styling';
...
export default function Edit( props ) {
	...
	const generatedCSS = useMemo( () => {
		...
		cssGenerator.addClassStyles(
			'{{WRAPPER}}',
			getVideoContainerCSS( attributes ),
			getVideoContainerCSS( attributes, 'Tablet' ),
			getVideoContainerCSS( attributes, 'Mobile' )
		);

		// eslint-disable-next-line
		let editorInlineCSSExtend = applyFilters(
			`ablocks.video.editorInlineCSS`,
			null,
			attributes,
			cssGenerator
		);
