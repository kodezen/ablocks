...
import { getIconButtonCSS, getIconTextCSS } from './styling';
import './editor.scss';
...
export default function Edit( props ) {
	...
	const generatedCSS = useMemo( () => {
		...
		cssGenerator.addClassStyles(
			...
			getIconButtonCSS( attributes ),
			getIconButtonCSS( attributes, 'Tablet' ),
			getIconButtonCSS( attributes, 'Mobile' )
		);
		cssGenerator.addClassStyles(
			`{{WRAPPER}} .storeengine-mini-cart>button.storeengine-mini-cart-link>.storeengine-min-cart-count`,
			getIconTextCSS( attributes ),
			getIconTextCSS( attributes, 'Tablet' ),
			getIconTextCSS( attributes, 'Mobile' )
		);
