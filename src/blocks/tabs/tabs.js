class ABlocksTabs {
	constructor( element, editor = true, settings = {} ) {
		...
		this.tabsTopHeight =
			this.tabs.getBoundingClientRect().top + window.scrollY;
		...
		if (
			this.editor === false &&
			this.settings.scrollChange &&
			this.settings.tabMenuPosition !== 'top' &&
			this.settings.tabMenuPosition !== 'bottom'
		) {
	...
	getDataSettings() {
		return {
			initialOpen:
				parseInt( this.tabs.getAttribute( 'data-initial-open' ) ) || 0,
			autoChange:
				this.tabs.getAttribute( 'data-enable-auto-change' ) === 'true',
			scrollChange:
				this.tabs.getAttribute( 'data-enable-scroll-change' ) ===
				'true',
			enableHoverSwitch:
				this.tabs.getAttribute( 'data-enable-hover-switch' ) === 'true',
			tabMenuPosition: this.tabs.getAttribute( 'data-tab-menu-position' ),
			activeDuration:
				parseInt(
					this.tabs.getAttribute( 'data-tab-active-duration' )
				) || 10000,
		};
	}
	getElements() {
	...
	isActiveTab( tabIndex ) {
		const { activeTitle } = this.settings.classes;
		const tabTitle = this.elements.tabTitles.find(
			( tab ) => tab.getAttribute( 'data-tab' ) === tabIndex
		);
	...
	deactivateTab( tabIndex ) {
		const { activeTitle, activeContent } = this.settings.classes;
		const tabTitle = this.elements.tabTitles.find(
			( tab ) => tab.getAttribute( 'data-tab' ) === tabIndex
		);

		const tabContent = this.elements.tabContents.find(
			( tab ) => tab.getAttribute( 'data-tab-id' ) === tabIndex
		);
		if ( tabTitle ) {
	...
	activateTab( tabIndex ) {
		...
		const progressBar = this.elements.tabTitles[ tabIndex ].querySelector(
			this.settings.selectors.progressBar
		);
		if ( tabTitle ) {
	...
	defaultActiveTab() {
		...
		const { activeTitle, activeContent } = this.settings.classes;
		const checkActiveTitle = this.elements.tabTitles.findIndex( ( tab ) =>
			tab.classList.contains( activeTitle )
		);
		const checkActiveContent = this.elements.tabContents.findIndex(
			( tab ) => tab.classList.contains( activeContent )
		);
		if (
			( initialOpen > 0 && checkActiveTitle === -1 ) ||
			( initialOpen > 0 && checkActiveContent === -1 )
		) {
	...
	startAutoChange() {
		this.autoChangeInterval = setInterval( () => {
			...
			const nextTabIndex =
				( activeTabIndex + 1 ) % this.elements.tabTitles.length;
	...
	startScrollChange() {
		const onScroll = () => {
			...
			this.elements.tabTitles.forEach( ( tabTitle, index ) => {
				const tabTop =
					tabTitle.getBoundingClientRect().top + window.scrollY;
	...
	getActiveTabIndex() {
		const { activeTitle } = this.settings.classes;
		const activeTab = this.elements.tabTitles.findIndex( ( tab ) =>
			tab.classList.contains( activeTitle )
		);
