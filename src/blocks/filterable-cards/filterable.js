class FilterableCards {
	constructor( element ) {
		...
		if ( this.element ) {
			this.filterButtons = this.element.querySelectorAll(
				'.filterable-filter-button'
			);
			this.cardsItems = [
				...this.element.querySelectorAll(
					'.ablocks-block--filterable-cards-item'
				),
			];
			this.showMoreButton = this.element.querySelector(
				'.filterable-cards-showMore-button'
			);
			this.searchInput = this.element.querySelector(
				'.filterable-searchInput'
			);
			this.searchToggleBtn = this.element.querySelector(
				'.filterable-search-toggle-btn'
			);
			this.filterListDropdown = this.element.querySelector(
				'.filterable-cards_filter-list-dropdown'
			);
			this.filterSearchNoItemText = this.element.querySelector(
				'.filterable-search-item-not-found-text'
			);
			this.searchFilterS = this.element.querySelector(
				'.filterable-search-select'
			);
			this.firstCategory =
				this.filterButtons[ 0 ]?.dataset.category?.toLowerCase() ||
				'all';
			this.showItemNumber = parseInt(
				this.element?.getAttribute( 'itemShows' ) || '0',
				10
			);
			...
			this.perPageShowCardNumber = parseInt(
				this.showMoreButton?.dataset.perPage || '0',
				10
			);
			this.noMoreText =
				this.showMoreButton?.dataset.noItemText || 'No more items.';
			this.loadMoreText =
				this.showMoreButton?.dataset.moreButtonText || 'Show More';
			this.currentlyShown = this.showItemNumber;
			this.animationDuration = parseInt(
				this.element?.getAttribute( 'data-duration' ) || '500',
				10
			);
			this.enableFilter =
				this.element?.getAttribute( 'data-enableFilter' ) === 'true';
			this.init();
		}
	}
	...
	filterCards( searchTerm, category ) {
		...
		this.cardsItems.forEach( ( item ) => {
			...
			const isMatch = searchTerm
				? headingText.includes( searchTerm.toLowerCase() )
				: true;
			const isMatchingCategory =
				category === this.firstCategory || itemCategory === category;

			this.toggleItem(
				item,
				( shouldSearch ? isMatch : true ) &&
					isMatchingCategory &&
					itemsShown < this.currentlyShown
			);
		...
		if ( this.enableFilter ) {
			this.filterSearchNoItemText.style.display = itemsShown
				? 'none'
				: 'block';
	...
	toggleShowMoreButton( category, searchTerm = '' ) {
		...
		const visibleItems = this.cardsItems.filter( ( item ) => {
			...
			const isHeadingMatch = shouldSearch
	...
	handleButtonClick( event ) {
		...
		this.filterButtons.forEach( ( el ) =>
			el.classList.remove( 'filterable-filter-button-active' )
		);
	...
	handleShowMoreClick() {
		...
		const activeButton =
			[ ...this.filterButtons ].find( ( button ) =>
				button.classList.contains( 'filterable-filter-button-active' )
			) || this.filterButtons[ 0 ];
		if ( activeButton ) {
			const category =
				activeButton.dataset.category?.toLowerCase() || 'all';
	...
	toggleSearchDropdown() {
		this.searchToggleBtn?.addEventListener( 'click', () => {
			const isHidden =
				this.filterListDropdown.style.display === 'none' ||
				! this.filterListDropdown.style.display;
