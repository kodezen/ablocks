document.addEventListener( 'DOMContentLoaded', () => {
	...
	function initSearchBlock( searchForm ) {
		...
		const searchResultContainer = searchForm
			.closest( '.ablocks-block--search-bar' )
			?.querySelector( '.ablocks-block--search-result' );
		if ( searchResultContainer ) {
			searchResultContainer.classList.add(
				'ablocks-block--search-empty-result'
			);
		}
	...
	document
		.querySelectorAll( '.ablocks-block--search-form' )
		.forEach( ( form ) => initSearchBlock( form ) );
	...
	async function executeSearch(
		...
		const source = searchForm.querySelector(
			'.ablocks-block--search-source'
		)?.value;
		const current_page_id = searchForm.querySelector(
			'.ablocks-block--search__current-post-id'
		)?.value;
		...
		try {
			...
			if ( data.success ) {
				...
				searchResultContainer.innerHTML =
					'<p class="ablocks-block--search-result__list-no-data">No results available for your search</p>';
				searchResultContainer.classList.remove(
					'ablocks-block--search-empty-result'
				);
			}
			...
			searchResultContainer.innerHTML =
				'<p class="ablocks-block--search-result__list-no-data">An error occurred during search. Please try again.</p>';
			searchResultContainer.classList.remove(
				'ablocks-block--search-empty-result'
			);
		} finally {
	...
	document.addEventListener( 'click', ( event ) => {
		const allSearchBars = document.querySelectorAll(
			'.ablocks-block--search-bar'
		);

		allSearchBars.forEach( ( searchBar ) => {
			const resultContainer = searchBar.querySelector(
				'.ablocks-block--search-result'
			);
