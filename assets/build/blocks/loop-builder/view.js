...
/***/ "./src/blocks/loop-builder/loop.js":
/*!*****************************************!*\
  !*** ./src/blocks/loop-builder/loop.js ***!
  \*****************************************/
...
class LoopBuilder {
  constructor(element, options = {}) {
    ...
    this.filterButtons = element.querySelectorAll('.ablocks-loop-term-filter');
    this.loadMoreButton = element.querySelector('.ablocks-loop-load-more__text');
    this.noMoreText = this.loadMoreButton?.dataset?.noItemText || 'No more items.';
    this.loadMoreText = this.loadMoreButton?.dataset?.moreButtonText || 'Show More';
    const activeFilter = element.querySelector('.ablocks-loop-term-filter--active');
    this.loadMorePostId = parseInt(this.loadMoreButton?.dataset?.postId) || parseInt(activeFilter?.dataset?.postId) || window.ablocks_current_post_id || 0;
    ...
    // added archive
    this.is_archive = this.loadMoreButton?.dataset?.isArchive === 'true' || false;
    this.archive_post_type = this.loadMoreButton?.dataset?.archivePostType || null;
    this.blockId = this.getBlockId(element);
    this.attachEvents();
    element.dataset.initialized = 'true';
  }
  getBlockId(element) {
    return [...element.classList].find(c => c.startsWith('ablocks-block-') && c.length > 20)?.replace('ablocks-block-', '') || null;
  }
  attachEvents() {
  ...
  handleFilterClick = async e => {
    ...
    clicked.style.width = clicked.offsetWidth - parseFloat(s.paddingLeft) - parseFloat(s.paddingRight) - parseFloat(s.borderLeftWidth) - parseFloat(s.borderRightWidth) + 'px';
    clicked.style.height = clicked.offsetHeight - parseFloat(s.paddingTop) - parseFloat(s.paddingBottom) + 'px';
    clicked.innerHTML = '<span class="ablocks-loop-loading-spinner"></span>';
    this.filterButtons.forEach(btn => btn.classList.remove('ablocks-loop-term-filter--active'));
    ...
    this.loadMorePostId = parseInt(clicked.dataset.postId) || window.ablocks_current_post_id;
    window.ablocks_current_post_id = this.loadMorePostId;
    this.currentPage = 1;
    // added archive
    window.ablocks_is_archive = clicked.dataset.isArchive === 'true' || window.ablocks_is_archive;
    window.ablocks_archive_post_type = clicked.dataset.archivePostType || window.ablocks_archive_post_type;
    const beforeItemsCount = this.lastTermId === this.termId ? document.querySelectorAll(`.ablocks-block-${this.blockId} .ablocks-loop-template-item`).length : 0;
  ...
  handleLoadMoreClick = () => {
    ...
    this.isLoading = true;
    this.loadingState();
    const active = this.element.querySelector('.ablocks-loop-term-filter--active');
    if (active) {
    ...
    // Load More term id
    if (!this.termId) {
      if (window.ablocks_current_term_id !== this.loadMoreButton?.dataset?.termId) {
        window.ablocks_current_term_id = this.loadMoreButton?.dataset?.termId;
      }
      this.termId = this.loadMoreButton?.dataset?.termId || window.ablocks_current_term_id;
    }
    window.ablocks_current_post_id = this.loadMorePostId || window.ablocks_current_post_id;
    this.currentPage++;
    // added archive
    window.ablocks_is_archive = this.loadMoreButton?.dataset?.isArchive === 'true' || window.ablocks_is_archive;
    window.ablocks_archive_post_type = this.loadMoreButton?.dataset?.archivePostType || window.ablocks_archive_post_type;
    const beforeItems = document.querySelectorAll(`.ablocks-block-${this.blockId} .ablocks-loop-template-item`).length;
  ...
  async fetchContent(beforeItems = 0) {
    const payload = {
      ...
      loop_template_block_id: this.getBlockId(this.element.parentElement?.querySelector('.ablocks-block--loop-template')),
    ...
    try {
      const res = await (0,_Utils_helper__WEBPACK_IMPORTED_MODULE_0__.makeRequest)(payload);
      const html = res?.data?.data?.html;
      if (html) {
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = html;
        this.element.replaceWith(...tempDiv.childNodes);
        const newLoopBuilder = document.querySelector(`.ablocks-block-${this.blockId}.ablocks-block--loop-builder`);
        if (newLoopBuilder) {
          const afterItems = document.querySelectorAll(`.ablocks-block-${this.blockId} .ablocks-loop-template-item`).length;
          const newLoadMoreText = newLoopBuilder.querySelector('.ablocks-loop-load-more__text');
          if (newLoadMoreText && window.ablocks_current_post_id) {
            newLoadMoreText.dataset.postId = window.ablocks_current_post_id;
            newLoadMoreText.innerText = afterItems <= beforeItems ? this.noMoreText : this.loadMoreText;
          }
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (LoopBuilder);
...
(() => {
/*!*****************************************!*\
  !*** ./src/blocks/loop-builder/view.js ***!
  \*****************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _loop__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./loop */ "./src/blocks/loop-builder/loop.js");

document.addEventListener('DOMContentLoaded', () => {
  const loopBuilders = document.querySelectorAll('.ablocks-block--loop-builder');
  loopBuilders.forEach(element => {
    new _loop__WEBPACK_IMPORTED_MODULE_0__["default"](element);
  });
});
})();
